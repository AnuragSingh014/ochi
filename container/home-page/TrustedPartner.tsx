      ...
      <div className="max-w-7xl mx-auto text-center">
        ...
        >
          {logos.map((logo, index) => (
            <motion.div
              key={index}
              className="bg-white rounded-xl shadow-lg p-6 flex justify-center items-center h-32"
              whileHover={{ scale: 1.1 }}
              transition={{ type: "spring", stiffness: 300 }}
            >
              <img src={logo.src} alt={logo.alt} className="h-20 max-w-full object-contain" />
            </motion.div>
          ))}
        </motion.div>
      </div>
